...
struct PhotoFilterService {
    ...
    static func apply(labels: Set<String>, ratings: Set<Int>,
                      to photos: [PhotoItem]) -> [PhotoItem] {
        var result = photos

        if !labels.isEmpty {
            result = result.filter { photo in
                if labels.contains("Rejected") && photo.toDelete { return true }
                let photoLabel = photo.xmp?.label ?? ""
                if labels.contains("No Label") && photoLabel.isEmpty && !photo.toDelete { return true }
    ...
    static func availableLabels(from photos: [PhotoItem]) -> [String] {
        ...
        for photo in photos {
            if photo.toDelete { hasToDelete = true }
            if let label = photo.xmp?.label, !label.isEmpty { labelSet.insert(label) }
        }
        var result: [String] = []
        if !labelSet.isEmpty { result.append("No Label") }
        for label in ["Select", "Second", "Approved", "Review", "To Do"] where labelSet.contains(label) {
            result.append(label)
        }
        if hasToDelete { result.append("Rejected") }
        return result
    }
}
